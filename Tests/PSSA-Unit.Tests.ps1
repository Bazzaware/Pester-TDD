Param(
    ...
    [string]$TestLocation = "$PSScriptRoot\..\Modules"
)
...
foreach ($Severity in $Severities) { 
    ...
    Describe "Testing PSSA $Severity Rules" -Tag $Severity {
        ...
        It "<RuleName>" -TestCases ($Rules | Where-Object Severity -eq $Severity) {
            ...
            #Test all scripts for the given rule and if there is a problem display this problem in a nice an readable format in the debug message and let the test fail
